...
import { X, Mail, Lock, User as UserIcon, ArrowRight, Loader2, AlertCircle, Sparkles } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const { login, register, loginWithGoogle, loginAsGuest, isLoading } = useAuth();
      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-3.5">
          ...
          <button
            type="submit"
            disabled={isLoading}
            className="w-full py-3 rounded-2xl bg-gradient-to-r from-sky-500 to-blue-600 hover:from-sky-600 hover:to-blue-700 text-white font-extrabold text-sm shadow-lg shadow-sky-500/25 transition-all active:scale-[0.98] flex items-center justify-center gap-2 mt-2"
          >
            {isLoading ? (
        ...
        <div className="space-y-2">
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-2.5 px-4 rounded-2xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-bold text-xs shadow-sm transition-all flex items-center justify-center gap-2.5"
          >
          ...
          <button
            ...
            disabled={isLoading}
            className="w-full py-2.5 px-4 rounded-2xl bg-sky-50 hover:bg-sky-100 border border-sky-200 text-sky-800 font-bold text-xs transition-all flex items-center justify-center gap-2"
          >
